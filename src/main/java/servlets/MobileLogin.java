...
public class MobileLogin extends HttpServlet {
  ...
  private static final Logger log = LogManager.getLogger(MobileLogin.class);

  /**
   * Initiated by login.jsp. Once this post request has been completely processed, the user will be
   * logged in, the account will be one count closer to been temporarily been locked or will be
   * locked out temporarily. This method takes the credentials submitted and determines if they are
   * correct. If they are correct, a session is prepared for the user and they are assigned a CSRF
   * token.
   *
  ...
  public void doPost(HttpServletRequest request, HttpServletResponse response)
    ...
    try {
      ...
      if (user != null && !user[0].isEmpty()) {
        ...
        // Removing user from kick list. If they were on it before, their suspension must have ended
        // if they DB authentication Succeeded
