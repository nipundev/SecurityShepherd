...
/**
 * Unvalidated Redirects and Forwards Lesson <br>
 * <br>
...
public class UnvalidatedForwardsLesson extends HttpServlet {
  ...
  private static final Logger log = LogManager.getLogger(UnvalidatedForwardsLesson.class);
  ...
  /**
   * User submission is parsed for a valid URL. This is then used to construct a URL object. This
   * URL object is then checked to ensure a valid attack
   *
  ...
  public void doPost(HttpServletRequest request, HttpServletResponse response)
    ...
    ResourceBundle bundle =
        ResourceBundle.getBundle("i18n.servlets.lessons.unvalidatedRedirect", locale);
      ...
      if (Validate.validateSession(ses)) {
        ...
        if (Validate.validateTokens(tokenCookie, tokenParmeter)) {
          ...
          try {
            ...
            validSolution =
                csrfUrl.getQuery().toLowerCase().startsWith(("to=").toLowerCase()) && validSolution;
            if (!validSolution) {
              log.debug("Invalid Solution: Bad Query or Above");
            }
            if (validSolution) {
              log.debug(
                  "Redirect URL Correct: Now checking the Redirected URL for valid CSRF Attack");
              int csrfStart = 0;
              int csrfEnd = 0;
