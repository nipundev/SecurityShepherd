...
public class EnableCheats extends HttpServlet {
  ...
  private static final Logger log = LogManager.getLogger(EnableCheats.class);

  /**
   * If this functionality is validly called by an administrator, the cheat sheets will become or
   * remain unavailable.
   *
   * @param csrfToken
   */
    ...
    if (Validate.validateAdminSession(ses, tokenCookie, tokenParameter)) {
      ...
      if (Validate.validateTokens(tokenCookie, tokenParameter)) {
        ...
        if ("true".equalsIgnoreCase(enableFor)) {
          ...
          out.write(
              "<h2 class='title'>Cheat Sheets Enabled</h2>"
                  + "<p>Cheat Sheets have been enabled for all Security Shepherd Users</p>");
        } else {
          CheatSheetStatus.enableForAdminsOnly();
          log.debug("Cheat Sheets Enabled");
          out.write(
              "<h2 class='title'>Cheat Sheets Enabled</h2>"
