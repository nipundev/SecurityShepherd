...
public class OpenOrCloseByCategory extends HttpServlet {
  ...
  private static final Logger log = LogManager.getLogger(OpenOrCloseByCategory.class);
      ...
      if (Validate.validateTokens(tokenCookie, tokenParmeter)) {
        ...
        if (validData && request.getParameterValues("toOpenOrClose[]") != null) {
          ...
          if ("open".equalsIgnoreCase(openOrClose)) {
            htmlOutput +=
                "<p>The categories selected have been opened and are now available to users to"
                    + " access.</p>";
          } else {
            htmlOutput +=
                "<p>The categories selected have been closed and are no longer available for users"
                    + " to access or submit solutions for.</p>";
          }
        } else {
